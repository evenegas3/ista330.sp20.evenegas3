...
function getLabels(imageId) {
  ...
  return postgrePool.query('SELECT name FROM word where imageid = $1' [imageId])
  .then(result => result.rows);
...
function saveLabel(name, x, y, number, imageId) {
  ...
  return postgrePool.query('INSERT INTO word(name, x, y, number, imageID) VALUES(“$1”,“$2”, "$3", "$4") returning id' [name, x, y, number, imageId])
  .then(result => result.rows[0].name);

  // return postgrePool.query('INSERT INTO word(name, x, y, number, imageID) VALUES(“${name}”,“${x}”, "${y}", "${imageId}") returning id')
  // .then(result => result.rows[0].name);
